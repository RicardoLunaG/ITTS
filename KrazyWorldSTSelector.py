...
from krazyworld import KrazyGridWorld
...
def UssefulnessMeasure(source_task, EPOCS = 5, NUMBER_EPISODES = 10, test_number= 0):
    ...
    @wrap_experiment(log_dir="./LoadDir/KrazyWorld/TaskST"+str(source_task),snapshot_mode="none",snapshot_gap= 0,use_existing_dir=True,name="TaskSTTask"+str(source_task))
        ...
        for _ in range(EPOCS):
            ...
            with tf.compat.v1.Session() as sess:
                ...
                with LocalTFRunner(snapshot_config=ctxt,sess=sess) as runner:
                    ...
                    saved_dir = "./Saved_Models/KrazyWorld/Task"+str(source_task)
...
def extract_action_prob(test_number, task = 0,number_test_inputs=20): 
    ...
    action_probs = []             
    @wrap_experiment(log_dir="./LoadDir/KrazyWorld/TestST"+str(task),snapshot_mode="none",snapshot_gap= 0,use_existing_dir=True,name="TestSTTask"+str(task))
    def prob_exp(ctxt):
        tf.compat.v1.reset_default_graph()
        with tf.compat.v1.Session() as sess:
            
            with LocalTFRunner(snapshot_config=ctxt,sess=sess) as runner:
                saved_dir = "./LoadDir/KrazyWorld/Task"+str(task)
