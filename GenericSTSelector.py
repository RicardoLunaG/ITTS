...
#Before using this class train invididual tasks using https://github.com/rlworkgroup/garage and save a checkpoint to be loaded here for each task.
#Parameters and model specifications are loaded when runner.restore is called.

def UssefulnessMeasure(source_task, EPOCS = 5, NUMBER_EPISODES = 10, test_number= 0):
        

    validationTasks = [] #Validation Tasks, append the tasks that will be used as validation tasks from the desired domain.
        ...
        for _ in range(EPOCS):
            ...
            with tf.compat.v1.Session() as sess:
                ...
                with LocalTFRunner(snapshot_config=ctxt,sess=sess) as runner:
                    ...
                    saved_dir = "./Saved_Models/Domain/Task"+str(source_task) #Route where the garage model is saved, all parameters of the specified model are loaded.
                    ...
                    runner.restore(from_dir=saved_dir,env = env) #Restoring the Model
...
def extract_action_prob(test_number, task = 0,number_test_inputs=20): 
    ...
    testing_inputs = [] #Fill the list with the states samples of the domain used.
    
    action_probs = []             
    @wrap_experiment(log_dir="./LoadDir/Domain/TestST"+str(task),snapshot_mode="none",snapshot_gap= 0,use_existing_dir=True,name="TestSTTask"+str(task))
    def prob_exp(ctxt):
        tf.compat.v1.reset_default_graph()
        with tf.compat.v1.Session() as sess:
...
def main(load_model = False):
    ...
    taskRange = 40 #Number of Tasks to be used
    starting_task = 0 #Starting Task, in case we want to start from a task different than the initial one.
    dif_value = 0 #Difference Value to use on the KL-Divergence calculation
